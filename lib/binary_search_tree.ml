...
struct
  ...
  let rec _member x = function
    | E -> false
    | T (left, y, right) ->
      if Element.lt x y
      then _member x left
      else if Element.lt y x
      then _member x right
      else (* x = y *) true

  let rec _insert x set = match set with
    | E -> T (E, x, E)
    | T (left, y, right) ->
      if Element.lt x y
      then T (_insert x left, y, right)
      else if Element.lt y x
      then T (left, y, _insert x right)
      else set

  (* Exercise 2.2: improve member *)
  ...
  *)
  ...
  module With_duplication = struct
    ...
    let rec member_with_candidate ~candidate x = function
      | E -> Element.eq candidate x
      | T (left, y, right) ->
        if Element.lt x y
        then member_with_candidate ~candidate x left
  ...
  (* Exercise 2.3: improve insertion of an existing element *)
  ...
  (* Exercise 2.4: improve insert, again *)
